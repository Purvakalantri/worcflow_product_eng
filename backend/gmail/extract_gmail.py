...
# from connect.save_commits import get_last_sync_time
...
def extract_email(service):
    try:
        ...
        two_days_ago= roday - timedelta(days=2)
        # three_days_ago = today - timedelta(days=3)

        query_inbox=f"after:{two_days_ago} category:Primary"
        query_sent=f"after:{two_days_ago}"

        logger.info(f"Inbox query: {query_inbox}")
        logger.info(f"Sent query: {query_sent}")
        ...
        logger.info(f"Inbox messages fetched: {len(inbox_messages)}")
        logger.info(f"Sent messages fetched: {len(sent_messages)}")
        ...
        for message in inbox_messages:
            ...
            logger.info(f"Fetched inbox email: "f"{email['date']} | {email['subject']}")
            emails.append(email)
        ...
        for message in sent_messages:
            ...
            logger.info(f"Fetched sent email: "f"{email['date']} | {email['subject']}")
            emails.append(email)
...
def extract_header(service, message_id):
    try:
        ...
        logger.exception(f"Failed to extract email: {message_id}")
        raise
