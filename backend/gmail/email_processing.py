...
def process_emails(threads, connected_user_email):
    ...
    try:
        ...
        for thread in threads:
            ...
            # print("__________Thread ID____________:", thread_id)
            ...
            if result["has_commitment"] is True:
                ...
                if data.get("due_date"):
                    data["due_date"] = data["due_date"][:10]
        ...
        logger.info(f"Active commitments found: {len(commitments)}")

        return commitments

    except Exception as e:
        logger.error(f"Failed while processing emails: {e}")
        raise